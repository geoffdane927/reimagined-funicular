...
let sourceCanvas=null,sourceCtx=null,modelCanvas=null,modelCtx=null;
...
function resizeForModel(image){
  if(!modelCanvas){
    ...
    modelCanvas.width=SIZE;modelCanvas.height=SIZE;modelCtx=modelCanvas.getContext('2d',{willReadFrequently:true});
  }
  if(!sourceCanvas){
    sourceCanvas=typeof OffscreenCanvas!=='undefined'?new OffscreenCanvas(image.width,image.height):document.createElement('canvas');
    sourceCanvas.width=image.width;sourceCanvas.height=image.height;sourceCtx=sourceCanvas.getContext('2d',{willReadFrequently:true});
  }
  ...
  modelCtx.drawImage(sourceCanvas,0,0,SIZE,SIZE);
  return modelCtx.getImageData(0,0,SIZE,SIZE);
}

function imageToTensor(image){
  const src=resizeForModel(image).data,x=new Float32Array(3*SIZE*SIZE);
...
function alignWindow(localCenters,globalCenters,localRotation,globalRotation){
  const A=matMul(transpose(globalRotation),localRotation);
  const ratios=[];
...
export async function reconstructVideo(frames,{pointStride=6,windowStep=1,splatSize=1,onProgress}={}){
  ...
  const session=await getSession(onProgress),inputName=session.inputNames[0],out=[];
  const globalCenters=new Map(),globalRotations=new Map(),starts=[];
  ...
  for(let wi=0;wi<starts.length;wi++){
    ...
    if(wi>0){
      const common=[];for(let v=0;v<4;v++)if(globalCenters.has(start+v))common.push(v);
      if(common.length>=2){const lc=common.map(v=>localCenters[v]),gc=common.map(v=>globalCenters.get(start+v)),ref=common[0];T=alignWindow(lc,gc,localRots[ref],globalRotations.get(start+ref));}
    }
    const overlap=wi===0?0:Math.max(0,4-windowStep);
    for(let v=0;v<4;v++){
      const frameIndex=start+v,R=localRots[v],t=translationFromExtrinsic(ext,v*12);
      globalCenters.set(frameIndex,transformPoint(T,localCenters[v]));
      globalRotations.set(frameIndex,matMul(T.A,transpose(R)));
