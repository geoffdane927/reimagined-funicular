export class VideoSampler {
  ...
  async captureFrames(count,onProgress) {
    const frames=[];
    const v=this.video;
    if (!Number.isFinite(v.duration) || v.duration<=0) throw Error('The video has no usable duration.');
    for(let i=0;i<count;i++){
      const t=count===1?0:(i/(count-1))*Math.max(0,v.duration-0.05);
      await this.seek(t);
  ...
  async sample(count, stride, depthSpread, onProgress) {
    const frames=await this.captureFrames(count,onProgress);
    const out=[];
    const width=this.canvas.width,height=this.canvas.height;
    for(let f=0;f<frames.length;f++){
      const data=frames[f].data;
      const z=1+(f/Math.max(1,frames.length-1))*Math.max(0.1,depthSpread);
      for(let y=0;y<height;y+=stride) for(let x=0;x<width;x+=stride){
        ...
        out.push((x/(width-1)-.5)*4,(.5-y/(height-1))*3,z,.0045+brightness*.004,data[i]/255,data[i+1]/255,data[i+2]/255,.72+brightness*.25);
      }
    }
